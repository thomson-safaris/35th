(function ($) {
	...
	$('.sound').click(function() {
		...
		if (!iOS) {
			$('.sound').toggleClass('fa-volume-off fa-volume-up');
		}
	});
	...
	var headers = [];
	for (i = 1; i <= 17; i++) { 
		headers.push('#slide'+i+' header');
	}

	// get all break up sections
	var breakSections = [];
	for (i = 1; i <= 17; i++) { 
		breakSections.push('#cb'+i);
	}
	...
	if (!Modernizr.touch) {
	    ...
	    .to($('#intro header, .scroll-hint'), 0.2, {autoAlpha: 0, ease:Power1.easeNone})
	    	...
	    	// Parallax effect (Pin and Zoom text) on the slide with call outs.
	    	$.get('/ali-our-great-designer/imports/data-min.json', function(data) {
	    		var obj = data;
	    		for (var k in obj) {
	    			if (obj[k].pinTitle !== undefined) {
	    				if (obj[k].pinZoom) {
	    					var pinSceneZTl = new TimelineMax();
	    					pinSceneZTl
	    					.to($('#slide'+k+' h1'), 0.2, {autoAlpha: 0, ease:Power1.easeNone}, 1.5)
	    					.to($('#slide'+k+' section'), 0.2, {autoAlpha: 0, ease:Power1.easeNone}, 1.5)
	    					.set($('#slide'+k+' h1'), {text: obj[k].pinTitle})
	    					.set($('#slide'+k+' p'), {text: obj[k].pinContent})
	    					.to($('#slide'+k+' .bcg'), 0.6, {scale: 1.2, transformOrigin: '0% 0%', ease:Power0.easeNone})
	    					.fromTo($('#slide'+k+' h1'), 0.7, {y: '+=20'}, {y: 0, autoAlpha: 1, ease:Power1.easeOut}, '+=0.4')
	    					.fromTo($('#slide'+k+' section'), 0.6, {y: '+=20'}, {y: 0, autoAlpha: 1, ease:Power1.easeOut}, '-=0.6')
	    					.set($('#slide'+k+' h1'), {autoAlpha: 1}, '+=2.5');

	    					var pinSceneZ = new ScrollMagic.Scene({
	    						triggerElement: '#slide'+k, 
	    						triggerHook: 0,
	    						duration: "300%"
	    					})
	    					.setPin("#slide"+k)
	    					.setTween(pinSceneZTl)
	    					.addTo(controller);
	    				}else{
